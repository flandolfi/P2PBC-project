...
/**
 * This class provides protocol executors, that simulates the behavior of the Newscast algorithm on
 * a {@link Network} of {@link Correspondent}s.
 *
 * @param <T> the news data type used by the {@link it.unipi.di.p2pbc.newscast.core.Agent}s
 */
public class Coordinator<T> {
    ...
    /**
     * Creates a {@link Coordinator} instance that will run on a given {@link Network}, logging
     * statistics on the given {@link Logger}s, if any.
     *
     * @param network the {@link Network} on which the protocol will run
     * @param loggers the {@link Logger}s to be used during the execution of the algorithm
     */
    ...
    /**
     * Simulates the protocol for a given number of cycles. Optionally shuffles the network before
     * starting the simulation.
     *
     * @param steps   the number of cycles
     * @param shuffle if {@code true}, randomly permutes the network before the first cycle of the
     *                protocol
     */
