...
public class ScaleFreeNetwork<T> extends RandomNetwork<T> {
    ...
    /**
     * Constructs a scale-free network using the Barabási–Albert algorithm,
     * starting from a network of size 2. If a node does not connect to any
     * other node, it will be redrawn until it is connected.
    ...
    /**
     * Constructs a scale-free network using the Barabási–Albert algorithm,
     * starting from a network of size {@code initialSize}. If a node does not
     * connect to any other node, it will be redrawn until it is connected.
     * Notice that, due to the limited cache size, the actual network topology
     * may not be consistent with the expected one. Every added node will have
     * an agent provided by he given {@link AgentFactory}.
     *
    ...
    /**
     * Constructs a scale-free network using the Barabási–Albert algorithm,
     * starting from a network of size 2. If {@code redraw} is {@code true} and
     * a node does not connect to any other node, it will be redrawn until it is
     * connected.
     * Notice that, due to the limited cache size, the actual network topology
     * may not be consistent with the expected one. Every added node will have
     * an agent provided by he given {@link AgentFactory}.
     *
     * @param size the size of the network
     * @param redraw if {@code true} nd a node does not connect to any other
     *               node, it will be redrawn until it is connected. In any
     *               case, the algorithm may generate disconnected networks
     * @param agentFactory the custom {@link AgentFactory}
     */
    ...
    /**
     * Constructs a scale-free network using the Barabási–Albert algorithm,
     * starting from a network of size {@code initialSize}. If {@code redraw} is
     * {@code true} and a node does not connect to any other node, it will be
     * redrawn until it is connected.
     * Notice that, due to the limited cache size, the actual network topology
     * may not be consistent with the expected one. Every added node will have
     * an agent provided by he given {@link AgentFactory}.
     *
